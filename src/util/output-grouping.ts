import { OutputData, isStreamOutput, StreamOutputData } from '@runt/schema'
 ...
 */
export function groupConsecutiveStreamOutputs(outputs: OutputData[]): OutputData[] {
  ...
  while (i < outputs.length) {
    ...
    if (currentOutput.outputType !== 'stream' || !isStreamOutput(currentOutput.data)) {
    ...
    while (j < outputs.length) {
      ...
      if (nextOutput.outputType !== 'stream' || !isStreamOutput(nextOutput.data)) {
        break
      }
