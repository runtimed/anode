...
function createEnvFiles() {
  ...
  const docworkerEnvPath = join(projectRoot, "packages", "docworker", ".env");
...
`;
  ...
  // Create docworker .env (for local development)
  if (!existsSync(docworkerEnvPath)) {
    const docworkerEnvContent = `# Anode Docworker Environment
# Auto-generated by setup script
# NOTE: For local development only - production uses wrangler secrets

# Authentication token for sync backend (local development)
AUTH_TOKEN=insecure-token-change-me
`;
    writeFileSync(docworkerEnvPath, docworkerEnvContent);
...
function validateEnvironment() {
  ...
  // Validate docworker .env has AUTH_TOKEN
  if (!docworkerEnvContent.includes("AUTH_TOKEN=")) {
    console.error("❌ Missing AUTH_TOKEN in docworker .env");
    return false;
  }
