...
import { decodeImageData } from "./artifact-client.ts";
...
export class RuntimeAgent {
      ...
      onUpdate: (entries: readonly ExecutionQueueData[]) => {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            ourRuntime = activeRuntimes.find(
  ...
  private async processExecution(
    ...
    const context: ExecutionContext = {
      ...
      appendTerminal: (
        ...
        this.store.commit(
          events.terminalOutputAppended2({
            outputId,
            delta,
            id,
            sequenceNumber,
      ...
      appendMarkdown: (
        ...
        const id = crypto.randomUUID();
        this.store.commit(
          events.markdownOutputAppended2({
            id,
            outputId,
            delta,
            sequenceNumber,
  ...
  protected async processImageContent(
    ...
    try {
      ...
      const imageData = decodeImageData(content);
